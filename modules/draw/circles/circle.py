...
class Circle(Base):
  FP_TYPE = 'fp.draw.circles.circle'
  CIRCLE_DIMS = '3D'
  CIRCLE_BEVEL_DEPTH = 0.01
  CIRCLE_FILL_MODE = 'FULL'
  STROKE_COLOR = (0.15, 0.15, 0.15)
  ...
  def poll(self, context):
    return context.active_object and is_one_of_points(context.active_object)

  def create(self, context):
    circle = bpy.data.curves.new(name="Окружность", type="CURVE")                       # Create new curve datablock
    ...
    circle_mat = bpy.data.materials.new('ОбводкаДуги')                                  # Create new material
    circle_mat.diffuse_color = self.STROKE_COLOR
    circle.materials.append(circle_mat)                                                 # Add material to the obj
    obj = bpy.data.objects.new('Окружность', circle)                                    # Create new obj with datablock
