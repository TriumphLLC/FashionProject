...
class FP_PointForDart(bpy.types.Operator):
  '''
  ...
  '''
  bl_idname = "fp.point_for_dart"
  bl_label = "FP_PointForDart"

  @classmethod
  def poll(cls, context):
    return (not OAAT_LOCK) and PointForDart().poll(context)

  def execute(self, context):
    PointForDart().create(context)
    return {'FINISHED'}
    

class FP_PointForDartWire(bpy.types.Operator, FP_WireProto):
  '''
  Оператор рисования направляющей между двумя точками.
  '''
  bl_idname = 'fp.draw_point_for_dart'
  ...
  def on_before_finish(self):
    ...
    PointForDart.DEFAULT_EXPRESSION = rel_loc
    OAAT_LOCK = False
...
class FP_PointForDartW(bpy.types.Macro):
  '''
  ...
  '''
  bl_idname = "fp_point_for_dart_wire"
  bl_label = "Wired point for dart"


def define_macros():
  FP_PointForDartW.define('FP_OT_draw_point_for_dart')
  FP_PointForDartW.define('FP_OT_point_for_dart')
