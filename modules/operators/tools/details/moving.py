...
class FP_Moving(bpy.types.Operator):
  bl_idname = "fp.moving"
  bl_label = "FP Moving"
...
class FP_MovingStop(bpy.types.Operator):
  bl_idname = 'fp.moving_stop'
  bl_label = 'FP Moving Stop'
...
class FP_MovingTarget(bpy.types.Operator):
  bl_idname = 'fp.moving_target'
  bl_label = 'FP Moving Target'
...
class FP_MovingContourTarget(bpy.types.Operator):
  bl_idname = "fp.moving_contour_target"
  bl_label = "FP Moving Contour Target"
  ...
  def _move(self, context):
      ...
      if not exist:
        for point_in_contour in pairs:
          if obj.parent == point_in_contour[0]:
            ...
            print("obj = ", obj)
            distance = str(get_distance(loc(point_in_contour[1]), loc(obj)))
    ...
    for obj in pairs:
      ...
      for ob in bpy.data.objects:
        if ob.fp_type == Line.FP_TYPE and (ob.fp_deps[0] == obj[0].fp_id or ob.fp_deps[1] == obj[0].fp_id):
          exist += 1
        if exist > 2:
          break
      if exist <= 2:
    ...
    for line in bpy.data.objects:
      if line.fp_type == Line.FP_TYPE and (line.fp_deps[0] == 0 or line.fp_deps[1] == 0):
        line.select = True
        bpy.ops.object.delete()
...
class FP_MovingReflection(bpy.types.Operator):
  bl_idname = "fp.moving_reflection"
  bl_label = "FP Moving Reflection"
